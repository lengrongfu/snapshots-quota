...
// project ids.
func NewControl(basePath string, containerdRoot string) (*Control, error) {
	//
	...
	q := Control{
		...
		targetProject:     make(map[string]uint32),
	}
	...
	//
	err = q.findNextProjectID(filepath.Join(containerdRoot, "io.containerd.snapshotter.v1.overlayfs", "snapshots"), baseProjectID)
	if err != nil {
		return nil, err
	}
...
func (q *Control) GetProject(targetPath string) (uint32, bool) {
	q.RLock()
...
func (q *Control) SetQuotaByProject(targetPath string, quota Quota, projectID uint32) error {
...
func (q *Control) ClearQuota(projectID uint32) error {
	zeroQuota := Quota{Size: 0}
	if err := setProjectQuota(q.backingFsBlockDev, projectID, zeroQuota); err != nil {
		return errors.Wrap(err, "failed to clear quota limits")
	}
	return nil
}

func (q *Control) SetProject(targetPath string) (uint32, error) {
	q.RLock()
	projectID, ok := q.quotas[targetPath]
	q.RUnlock()
	if !ok {
